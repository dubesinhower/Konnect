...
| `activate_editor_context` | Capability-gate exact document/sheet activation, object reveal/centering, and view fitting; return typed unsupported behavior when no stable command plus active-context readback exists, without sending raw actions. |
| `resolve_cross_probe` | Resolve an exact schematic-symbol/PCB-footprint destination in either direction from stable saved footprint symbol-path linkage, while proving both explicit live document contexts and performing no unsupported activation or selection. |
